...
function loadUserTransactions(uid) {
  ...
  transactions = transactions.map((t) => ({ ...t, timestamp: t.timestamp || new Date(t.date).getTime() }));
}

function saveTransactions() {
  if (!currentUserUid) return;
  localStorage.setItem(`transactions_${currentUserUid}`, JSON.stringify(transactions));
}
...
function updateUI() {
  ...
  sorted.forEach((t) => {
    ...
    const categoryHTML =
      t.type === "expense" ? `<span class="separator"> — </span><span class="category">${t.category}</span>` : "";
